...
class MyGraph:
    ...
    def mean_degree (self, deg_type = "inout"): #vai devolver a média dos graus na árvore
        degs = self.all_degrees(deg_type)
        return sum(degs.values()) / float(len(degs))
    
    def prob_degree (self, deg_type = "inout"): #vai devolver a probabilidade de todos os graus na árvore ou ambos o grau é o numero de entrada  de saida
        degs = self.all_degrees(deg_type)
        res = {}
        for k in degs.keys():
            if degs[k] in res.keys():
    ...
    def mean_distances(self): #Vai devolver a média de distâncias entre todos os nódulos e a proporção de nós atingiveis a partir de um nó.
        tot = 0
        num_reachable = 0
        for k in self.graph.keys():
            distsk = self.reachable_with_dist(k)
