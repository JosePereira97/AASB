...
class MyGraph:
    ...
    def check_if_valid_path(self, p): #vai ver se o path é válido ou não
    ...
    def check_if_hamiltonian_path(self, p): #vai ver se exite um caminho que passe precisamente por todos os caminho uma vez e só uma vez
    ...
    def search_hamiltonian_path(self): #vai procurar um hamiltonian_Path para o grafo começando em todos os nodes
    ...
    def search_hamiltonian_path_from_node(self, start): #vai procurar um hamiltonian path a partir de um node chamado de start
        current = start
        visited = {start:0}
        path = [start]
        while len(path) < len(self.get_nodes()): #enquanto o caminho n tiver corrido os nodes todos
            ...
            if len(self.graph[current]) > nxt_index: #se o tamanho de ligações for maior que as ligações no current vértice implica que é impossivel continuar ir para else e dar pop()
                ...
                if nxtnode not in path: #se o next node n se encotrar no path já, vamos adicionálo e mudar o current node para este
                    ...
                    visited[nxtnode] = 0                    
                    current = nxtnode      
            else: #se o cumprimento do path for 1 ou menor implica que é impossivel ocorer um hamiltonian path
                if len(path) > 1: 
                    rmvnode = path.pop() #vai remover o ultimo visitado e mudar para o anterior
                    del visited[rmvnode]
                    current = path[-1]
                else: return None
        return path
